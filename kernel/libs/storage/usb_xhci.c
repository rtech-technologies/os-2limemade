...
void xhci_ring_init(xhci_ring_t* ring, uint32_t sz) {
    ...
    ring->phys = (uintptr_t)vmm_get_phys(p); ring->virt = (xhci_trb_t*)((uint64_t)p + get_hhdm_offset());
...
void xhci_on_interrupt(xhci_controller_t* hcd) {
    ...
    while (1) {
        ...
        hcd->ev.dequeue++; if (hcd->ev.dequeue >= 256) { hcd->ev.dequeue = 0; hcd->ev.cycle = !hcd->ev.cycle; }
    }
    hcd->run->interrupters[0].erDequeuePtr = (hcd->ev.ring_phys + hcd->ev.dequeue * 16) | XHCI_INT_ERDP_BUSY;
    hcd->run->interrupters[0].interruptPending = 0; spin_unlock(&hcd->lock);
}
void xhci_cmd_send(xhci_controller_t* hcd, void* trb) {
    xhci_trb_t* t = (xhci_trb_t*)trb; t->cycleBit = hcd->cmd.cycle; hcd->cmd.virt[hcd->cmd.enqueue++] = *t;
    if (hcd->cmd.enqueue >= hcd->cmd.max) { hcd->cmd.enqueue = 0; hcd->cmd.cycle = !hcd->cmd.cycle; }
    hcd->db[0].doorbell = 0; int timeout = 100000; while (timeout--) { xhci_on_interrupt(hcd); __asm__ volatile("pause"); }
}
...
int xhci_msc_read(void* priv, uint64_t lba, uint32_t count, void* buf) {
    xhci_device_t* dev = (xhci_device_t*)priv; xhci_controller_t* hcd = g_xhci_controllers[0]; if (!hcd || !dev) return -1;
    msc_cbw_t cbw = { .sig = 0x43425355, .tag = 1, .len = count * 512, .flg = 0x80, .cmdlen = 10 };
    cbw.cmd[0] = 0x28; cbw.cmd[2] = (lba>>24)&0xFF; cbw.cmd[3] = (lba>>16)&0xFF; cbw.cmd[4] = (lba>>8)&0xFF; cbw.cmd[5] = lba&0xFF;
    cbw.cmd[7] = (count>>8)&0xFF; cbw.cmd[8] = count&0xFF;
    xhci_trb_t setup = { .parameter = {(uint32_t)vmm_get_phys(&cbw), (uint32_t)(vmm_get_phys(&cbw) >> 32)}, .status = 31, .trbType = 1, .control = (1 << 6) }; xhci_cmd_send(hcd, &setup);
    xhci_trb_t data = { .parameter = {(uint32_t)vmm_get_phys(buf), (uint32_t)(vmm_get_phys(buf) >> 32)}, .status = count * 512, .trbType = 1, .control = (1 << 6) }; xhci_cmd_send(hcd, &data);
    msc_csw_t csw; xhci_trb_t status = { .parameter = {(uint32_t)vmm_get_phys(&csw), (uint32_t)(vmm_get_phys(&csw) >> 32)}, .status = 13, .trbType = 1, .control = (1 << 6) }; xhci_cmd_send(hcd, &status);
    if (csw.sig != 0x53425355 || csw.stat != 0) return -1; return 0;
}
void xhci_setup_device(xhci_controller_t* hcd, uint8_t port) {
    (void)port; xhci_trb_t en_slot = { .trbType = 9 }; xhci_cmd_send(hcd, &en_slot);
    xhci_device_t* dev = malloc(sizeof(xhci_device_t)); local_memset(dev, 0, sizeof(xhci_device_t));
    vdisk_node_t stick = { .name = "USB_STICK", .sector_size = 512, .read_lba = xhci_msc_read, .private_data = dev }; register_hardware_disk(stick);
}
bool xhci_bios_takeover(xhci_controller_t* hcd) {
    ...
    while (cap_ptr) {
        ...
        if ((cap & 0xFF) == 1) {
            *(volatile uint32_t*)cap_ptr |= (1 << 24); int timeout = 100;
            while ((*(volatile uint32_t*)cap_ptr & (1 << 16)) && timeout--) pit_wait_ms(1);
...
void xhci_init_controller(xhci_controller_t* hcd, uintptr_t base) {
    ...
    void* dcbaa_p = pmm_alloc(1); hcd->op->devContextBaseAddrArrayPtr = (uintptr_t)vmm_get_phys(dcbaa_p);
    xhci_ring_init(&hcd->cmd, 4096); hcd->op->cmdRingCtl = hcd->cmd.phys | 1; hcd->op->configure = hcd->cap->hcsParams1 & 0xFF;
    void* erst_p = pmm_alloc(1); xhci_event_ring_segment_table_entry_t* erst = (xhci_event_ring_segment_table_entry_t*)((uint64_t)erst_p + get_hhdm_offset());
    void* er_p = pmm_alloc(1); hcd->ev.ring_phys = (uintptr_t)vmm_get_phys(er_p);
    hcd->ev.ring = (xhci_event_trb_t*)((uint64_t)er_p + get_hhdm_offset()); local_memset(hcd->ev.ring, 0, 4096);
    erst[0].ringSegmentBaseAddress = hcd->ev.ring_phys; erst[0].ringSegmentSize = 256;
    hcd->ev.dequeue = 0; hcd->ev.cycle = true;
    hcd->run->interrupters[0].erstSize = 1; hcd->run->interrupters[0].erstBase = (uintptr_t)vmm_get_phys(erst_p);
    hcd->run->interrupters[0].erDequeuePtr = hcd->ev.ring_phys | XHCI_INT_ERDP_BUSY;
    ...
    for (int i=0; i<(int)(hcd->cap->hcsParams1>>24); i++) {
        hcd->ports[i].portSC |= (1<<9); hcd->ports[i].portSC |= (1<<4); if (hcd->ports[i].portSC & 1) xhci_setup_device(hcd, i+1);
    }
}
void usb_xhci_service(kernel_event_t event) {
    if (event == EVENT_INIT) {
        for (int b=0; b<256; b++) for (int s=0; s<32; s++) for (int f=0; f<8; f++) {
            uint32_t id = pci_config_read(b, s, f, 0x08);
            if (((id >> 24) & 0xFF) == 0x0C && ((id >> 16) & 0xFF) == 0x03 && ((id >> 8) & 0xFF) == 0x30) {
                pci_enable_master(b, s, f); uint32_t b0 = pci_config_read(b, s, f, 0x10); uintptr_t base = b0 & 0xFFFFFFF0;
                if ((b0 & 0x06) == 0x04) base |= ((uint64_t)pci_config_read(b, s, f, 0x14) << 32);
        ...
        extern void xhci_irq_stub(void); if (g_xhci_count) idt_set_descriptor(43, xhci_irq_stub, 0x8E);
    }
}
