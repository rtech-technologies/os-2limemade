...
typedef struct {
    nvme_regs_t* regs; uint32_t* doorbells; nvme_sqe_t* asq; nvme_cqe_t* acq;
    nvme_sqe_t* sq; nvme_cqe_t* cq; uint16_t asq_tail; uint16_t acq_head;
    uint16_t sq_tail; uint16_t cq_head; uint16_t acq_phase; uint16_t cq_phase;
} nvme_ctrl_t;
...
int nvme_submit_admin_cmd(nvme_sqe_t* cmd, nvme_cqe_t* res) {
    ...
    g_nvme.asq[g_nvme.asq_tail] = *cmd; g_nvme.asq_tail = (g_nvme.asq_tail + 1) % 64;
    g_nvme.doorbells[0] = g_nvme.asq_tail; int timeout = 1000000;
    while ((g_nvme.acq[g_nvme.acq_head].status & 1) != g_nvme.acq_phase && timeout--) __asm__ volatile("pause");
    if (timeout <= 0) return -1;
    *res = g_nvme.acq[g_nvme.acq_head]; g_nvme.acq_head = (g_nvme.acq_head + 1) % 64;
    if (g_nvme.acq_head == 0) g_nvme.acq_phase ^= 1;
    g_nvme.doorbells[1 << dstrd] = g_nvme.acq_head; return (res->status >> 1);
}
int nvme_read(void* priv, uint64_t lba, uint32_t count, void* buf) {
    (void)priv; uint32_t dstrd = (g_nvme.regs->cap >> 32) & 0xF;
    nvme_sqe_t cmd = { .cdw0 = 2, .nsid = 1, .prp1 = vmm_get_phys(buf), .cdw10 = (uint32_t)lba, .cdw11 = (uint32_t)(lba >> 32), .cdw12 = count - 1 };
    g_nvme.sq[g_nvme.sq_tail] = cmd; g_nvme.sq_tail = (g_nvme.sq_tail + 1) % 64;
    g_nvme.doorbells[2 << dstrd] = g_nvme.sq_tail; int timeout = 1000000;
    while ((g_nvme.cq[g_nvme.cq_head].status & 1) != g_nvme.cq_phase && timeout--) __asm__ volatile("pause");
    if (timeout <= 0) return -1;
    g_nvme.cq_head = (g_nvme.cq_head + 1) % 64; if (g_nvme.cq_head == 0) g_nvme.cq_phase ^= 1;
    g_nvme.doorbells[3 << dstrd] = g_nvme.cq_head; return 0;
}
    ...
    if (event == EVENT_INIT) {
        for (int b=0; b<256; b++) for (int s=0; s<32; s++) for (int f=0; f<8; f++) {
            ...
            if (((id >> 24) & 0xFF) == 0x01 && ((id >> 16) & 0xFF) == 0x08) {
                pci_enable_master(b, s, f); uint32_t b0 = pci_config_read(b, s, f, 0x10); uint32_t b1 = pci_config_read(b, s, f, 0x14);
                ...
                g_nvme.asq = (nvme_sqe_t*)(get_hhdm_offset() + (uintptr_t)pmm_alloc(1)); g_nvme.acq = (nvme_cqe_t*)(get_hhdm_offset() + (uintptr_t)pmm_alloc(1));
                local_memset(g_nvme.asq, 0, 4096); local_memset(g_nvme.acq, 0, 4096); g_nvme.acq_phase = 1; g_nvme.cq_phase = 1;
                g_nvme.regs->asq = vmm_get_phys(g_nvme.asq); g_nvme.regs->acq = vmm_get_phys(g_nvme.acq);
                g_nvme.regs->aqa = (63 << 16) | 63; g_nvme.regs->cc = (4 << 20) | (6 << 16) | 1; while(!(g_nvme.regs->csts & 1));
                g_nvme.sq = (nvme_sqe_t*)(get_hhdm_offset() + (uintptr_t)pmm_alloc(1)); g_nvme.cq = (nvme_cqe_t*)(get_hhdm_offset() + (uintptr_t)pmm_alloc(1));
                ...
                register_hardware_disk(node); return;
            }
        }
    }
}
