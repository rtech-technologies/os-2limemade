...
int ahci_read_sectors(void* priv, uint64_t lba, uint32_t count, void* buffer) {
    ...
    port->ci = 1;
    return ahci_wait_status(port, 0x01, 0, 1000000);
}
        ...
        for (int slot = 0; slot < 32; slot++) {
            for (int func = 0; func < 8; func++) {
                ...
                if (((class_info >> 24) & 0xFF) == 0x01 && ((class_info >> 16) & 0xFF) == 0x06) {
                    ...
                    int timeout = 1000;
                    while ((hba_base->ghc & (1 << 0)) && timeout--) pit_wait_ms(1);
                    hba_base->ghc |= (1 << 31);
                    ...
                    for (int p = 0; p < 32; p++) {
                        if (hba_base->pi & (1 << p)) {
                            ...
                            /* SOVEREIGN ORDER: Set addresses WHILE stopped */
                            uint64_t clb_p = vmm_get_phys(port_clb_virt[p]);
                            port->clb = (uint32_t)clb_p; port->clbu = (uint32_t)(clb_p >> 32);
                            uint64_t fb_p = vmm_get_phys(port_fb_virt[p]);
                            port->fb = (uint32_t)fb_p; port->fbu = (uint32_t)(fb_p >> 32);
                            ...
                            int link_wait = 1000;
                            while ((port->ssts & 0x0F) != 0x03 && link_wait--) pit_wait_ms(1);
                            if ((port->ssts & 0x0F) == 0x03) {
                                ...
                                int sig_wait = 1000;
                                while (port->sig == 0xFFFFFFFF && sig_wait--) pit_wait_ms(1);
                                if (port->sig == 0x00000101) {
                                    vdisk_node_t disk = { .name = "SATA_HDD", .sector_size = 512, .total_lba = 1024*1024*10, .partition_offset = 2048, .read_lba = ahci_read_sectors, .write_lba = ahci_write_sectors, .private_data = (void*)(uint64_t)p };
                                    register_hardware_disk(disk);
                                    vga_print("[AHCI] Port %d: SATA HDD online.\n", p);
                                    ahci_online = true;
                                } else if (port->sig == 0xEB140101) {
                                    vdisk_node_t cd = { .name = "SATA_CD", .sector_size = 2048, .read_lba = satapi_read_sectors, .eject = satapi_eject, .private_data = (void*)(uint64_t)p, .is_atapi = true };
                                    register_hardware_disk(cd);
