#include <include/rsl.h>

void* malloc(size_t size) {
    void* ptr = NULL;
    __asm__ volatile ("int $3" : : "a"((uint64_t)140), "b"((uint64_t)size), "c"((uint64_t)&ptr) : "memory");
    return ptr;
}

void free(void* ptr) {
    __asm__ volatile ("int $3" : : "a"((uint64_t)141), "b"((uint64_t)ptr) : "memory");
}

size_t strlen(const char* s) {
    size_t len = 0;
    while (s[len]) len++;
    return len;
}

void* memcpy(void* dest, const void* src, size_t n) {
    uint8_t* d = (uint8_t*)dest;
    const uint8_t* s = (const uint8_t*)src;
    for (size_t i = 0; i < n; i++) d[i] = s[i];
    return dest;
}

void* memset(void* s, int c, size_t n) {
    uint8_t* p = (uint8_t*)s;
    for (size_t i = 0; i < n; i++) p[i] = (uint8_t)c;
    return s;
}

int strcmp(const char* s1, const char* s2) {
    while (*s1 && (*s1 == *s2)) {
        s1++; s2++;
    }
    return *(uint8_t*)s1 - *(uint8_t*)s2;
}

char* strcpy(char* dest, const char* src) {
    char* d = dest;
    while ((*d++ = *src++));
    return dest;
}

void print(const char* s) {
    __asm__ volatile ("int $3" : : "a"((uint64_t)0), "b"((uint64_t)s) : "memory");
}

void set_color(color_t fg, color_t bg) {
    __asm__ volatile ("int $3" : : "a"((uint64_t)10), "b"((uint64_t)fg), "c"((uint64_t)bg) : "memory");
}

void* input(const char* prompt) {
    uint64_t res = 0;
    __asm__ volatile ("int $3" : "=a"(res) : "a"((uint64_t)11), "b"((uint64_t)prompt) : "memory");
    return (void*)res;
}

void retain(void* ptr) {
    __asm__ volatile ("int $3" : : "a"((uint64_t)20), "b"((uint64_t)ptr) : "memory");
}

void release(void* ptr) {
    __asm__ volatile ("int $3" : : "a"((uint64_t)21), "b"((uint64_t)ptr) : "memory");
}

void* str_create(const char* cstr) {
    uint64_t res = 0;
    __asm__ volatile ("int $3" : "=a"(res) : "a"((uint64_t)30), "b"((uint64_t)cstr) : "memory");
    return (void*)res;
}

const char* str_to_cstr(void* str) {
    uint64_t res = 0;
    __asm__ volatile ("int $3" : "=a"(res) : "a"((uint64_t)35), "b"((uint64_t)str) : "memory");
    return (const char*)res;
}

bool str_is_empty(void* str) {
    volatile bool res = false;
    __asm__ volatile ("int $3" : : "a"((uint64_t)32), "b"((uint64_t)str), "c"((uint64_t)&res) : "memory");
    return res;
}

void* str_concat(void* s1, void* s2) {
    uint64_t res = 0;
    __asm__ volatile ("int $3" : "=a"(res) : "a"((uint64_t)31), "b"((uint64_t)s1), "c"((uint64_t)s2) : "memory");
    return (void*)res;
}

int rsl_get_fb(rsl_fb_t* fb) {
    uint64_t res = 0;
    __asm__ volatile ("int $3" : "=a"(res) : "a"((uint64_t)202), "b"((uint64_t)fb) : "memory");
    return (int)res;
}

void rsl_user_create(const char* name, const char* pass, int* out_res) {
    uint64_t res = 0;
    __asm__ volatile ("int $3" : "=a"(res) : "a"((uint64_t)112), "b"((uint64_t)name), "c"((uint64_t)pass) : "memory");
    if (out_res) *out_res = (int)res;
}

void rsl_mount_vfs(int disk_id, const char* name, int* out_res) {
    uint64_t res = 0;
    __asm__ volatile ("int $3" : "=a"(res) : "a"((uint64_t)301), "b"((uint64_t)disk_id), "c"((uint64_t)name) : "memory");
    if (out_res) *out_res = (int)res;
}

int rsl_list_disks(void) {
    uint64_t count = 0;
    __asm__ volatile ("int $3" : "=a"(count) : "a"((uint64_t)101) : "memory");
    return (int)count;
}

int rsl_get_disk_info(int id, char* name, uint64_t* size) {
    uint64_t res = 0;
    __asm__ volatile ("int $3" : "=a"(res) : "a"((uint64_t)103), "b"((uint64_t)id), "c"((uint64_t)name), "d"((uint64_t)size) : "memory");
    return (int)res;
}

int rsl_partition_disk(int id) {
    uint64_t res = 0;
    __asm__ volatile ("int $3" : "=a"(res) : "a"((uint64_t)105), "b"((uint64_t)id) : "memory");
    return (int)res;
}

int rsl_format_disk(int id) {
    uint64_t res = 0;
    __asm__ volatile ("int $3" : "=a"(res) : "a"((uint64_t)104), "b"((uint64_t)id) : "memory");
    return (int)res;
}

void rsl_write(void* path, void* content) {
    __asm__ volatile ("int $3" : : "a"((uint64_t)120), "b"((uint64_t)path), "c"((uint64_t)content) : "memory");
}

vfs_handle_user_t rsl_open(void* path, const char* mode) {
    uint64_t res = 0;
    __asm__ volatile ("int $3" : "=a"(res) : "a"((uint64_t)122), "b"((uint64_t)path), "c"((uint64_t)mode) : "memory");
    return (vfs_handle_user_t)res;
}

int rsl_read(vfs_handle_user_t handle, void* buf, int len) {
    uint64_t res = 0;
    __asm__ volatile ("int $3" : "=a"(res) : "a"((uint64_t)123), "b"((uint64_t)handle), "c"((uint64_t)buf), "S"((uint64_t)len) : "memory");
    return (int)res;
}

void rsl_close(vfs_handle_user_t handle) {
    __asm__ volatile ("int $3" : : "a"((uint64_t)124), "b"((uint64_t)handle) : "memory");
}

void rsl_ls(void* path) { __asm__ volatile ("int $3" : : "a"((uint64_t)50), "b"((uint64_t)path) : "memory"); }
void rsl_cat(void* path) { __asm__ volatile ("int $3" : : "a"((uint64_t)51), "b"((uint64_t)path) : "memory"); }
void rsl_cd(void* path) { __asm__ volatile ("int $3" : : "a"((uint64_t)52), "b"((uint64_t)path) : "memory"); }
void rsl_mkdir(void* path) { __asm__ volatile ("int $3" : : "a"((uint64_t)53), "b"((uint64_t)path) : "memory"); }

bool rsl_exists(void* path) {
    uint64_t res = 0;
    __asm__ volatile ("int $3" : "=a"(res) : "a"((uint64_t)15), "b"((uint64_t)path) : "memory");
    return (bool)res;
}

void rsl_dispatch_command(char* line, void** curdir_ptr, bool* is_safe_ptr) {
    __asm__ volatile ("int $3" : : "a"((uint64_t)300), "b"((uint64_t)line), "c"((uint64_t)curdir_ptr), "d"((uint64_t)is_safe_ptr) : "memory");
}

void rsl_yield(void) { sys_yield(); }
void sys_yield(void) {
    __asm__ volatile ("int $3" : : "a"((uint64_t)2) : "memory");
}

void rsl_input(const char* prompt, char* buffer) {
    __asm__ volatile ("int $3" : : "a"((uint64_t)1), "b"((uint64_t)prompt), "c"((uint64_t)buffer) : "memory");
}

void gui_draw_rect(rsl_fb_t* fb, int x, int y, int w, int h, uint32_t color) {
    if (!fb || !fb->address) return;
    for (int i = 0; i < h; i++) {
        for (int j = 0; j < w; j++) {
            int px = x + j;
            int py = y + i;
            if (px >= 0 && (uint64_t)px < fb->width && py >= 0 && (uint64_t)py < fb->height) {
                uint32_t* pixel = (uint32_t*)(fb->address + py * fb->pitch + px * 4);
                *pixel = color;
            }
        }
    }
}

static const uint8_t font8x8_basic[128][8] = {
    [0x20] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },
    [0x21] = { 0x18, 0x18, 0x18, 0x18, 0x00, 0x00, 0x18, 0x00 },
    [0x22] = { 0x6C, 0x6C, 0x6C, 0x00, 0x00, 0x00, 0x00, 0x00 },
    [0x23] = { 0x36, 0x36, 0x7F, 0x36, 0x7F, 0x36, 0x36, 0x00 },
    [0x24] = { 0x18, 0x3E, 0x60, 0x3C, 0x06, 0x7C, 0x18, 0x00 },
    [0x25] = { 0x00, 0x62, 0x66, 0x0C, 0x18, 0x33, 0x23, 0x00 },
    [0x26] = { 0x38, 0x6C, 0x38, 0x76, 0xDC, 0xCC, 0x76, 0x00 },
    [0x27] = { 0x18, 0x18, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00 },
    [0x28] = { 0x0C, 0x18, 0x30, 0x30, 0x30, 0x18, 0x0C, 0x00 },
    [0x29] = { 0x30, 0x18, 0x0C, 0x0C, 0x0C, 0x18, 0x30, 0x00 },
    [0x2A] = { 0x00, 0x66, 0x3C, 0xFF, 0x3C, 0x66, 0x00, 0x00 },
    [0x2B] = { 0x00, 0x18, 0x18, 0x7E, 0x18, 0x18, 0x00, 0x00 },
    [0x2C] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x18, 0x30 },
    [0x2D] = { 0x00, 0x00, 0x00, 0x7E, 0x00, 0x00, 0x00, 0x00 },
    [0x2E] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x18, 0x18, 0x00 },
    [0x2F] = { 0x00, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x00 },
    [0x30] = { 0x3C, 0x66, 0x6E, 0x7E, 0x76, 0x66, 0x3C, 0x00 },
    [0x31] = { 0x18, 0x38, 0x18, 0x18, 0x18, 0x18, 0x3C, 0x00 },
    [0x32] = { 0x3C, 0x66, 0x06, 0x3C, 0x60, 0x66, 0x7E, 0x00 },
    [0x33] = { 0x3C, 0x66, 0x06, 0x1C, 0x06, 0x66, 0x3C, 0x00 },
    [0x34] = { 0x0C, 0x1C, 0x3C, 0x6C, 0x7E, 0x0C, 0x0C, 0x00 },
    [0x35] = { 0x7E, 0x60, 0x7C, 0x06, 0x06, 0x66, 0x3C, 0x00 },
    [0x36] = { 0x3C, 0x60, 0x7C, 0x66, 0x66, 0x66, 0x3C, 0x00 },
    [0x37] = { 0x7E, 0x06, 0x0C, 0x18, 0x30, 0x30, 0x30, 0x00 },
    [0x38] = { 0x3C, 0x66, 0x3C, 0x66, 0x66, 0x66, 0x3C, 0x00 },
    [0x39] = { 0x3C, 0x66, 0x66, 0x3E, 0x06, 0x66, 0x3C, 0x00 },
    [0x3A] = { 0x00, 0x18, 0x18, 0x00, 0x18, 0x18, 0x00, 0x00 },
    [0x3B] = { 0x00, 0x18, 0x18, 0x00, 0x18, 0x18, 0x30, 0x00 },
    [0x3C] = { 0x0C, 0x18, 0x30, 0x60, 0x30, 0x18, 0x0C, 0x00 },
    [0x3D] = { 0x00, 0x00, 0x7E, 0x00, 0x7E, 0x00, 0x00, 0x00 },
    [0x3E] = { 0x30, 0x18, 0x0C, 0x06, 0x0C, 0x18, 0x30, 0x00 },
    [0x3F] = { 0x3C, 0x66, 0x06, 0x0C, 0x18, 0x00, 0x18, 0x00 },
    [0x40] = { 0x3C, 0x66, 0x6E, 0x6A, 0x60, 0x62, 0x3C, 0x00 },
    [0x41] = { 0x18, 0x3C, 0x66, 0x7E, 0x66, 0x66, 0x66, 0x00 },
    [0x42] = { 0x7C, 0x66, 0x66, 0x7C, 0x66, 0x66, 0x7C, 0x00 },
    [0x43] = { 0x3C, 0x66, 0x60, 0x60, 0x60, 0x66, 0x3C, 0x00 },
    [0x44] = { 0x78, 0x6C, 0x66, 0x66, 0x66, 0x6C, 0x78, 0x00 },
    [0x45] = { 0x7E, 0x60, 0x60, 0x78, 0x60, 0x60, 0x7E, 0x00 },
    [0x46] = { 0x7E, 0x60, 0x60, 0x78, 0x60, 0x60, 0x60, 0x00 },
    [0x47] = { 0x3C, 0x66, 0x60, 0x6E, 0x66, 0x66, 0x3C, 0x00 },
    [0x48] = { 0x66, 0x66, 0x66, 0x7E, 0x66, 0x66, 0x66, 0x00 },
    [0x49] = { 0x3C, 0x18, 0x18, 0x18, 0x18, 0x18, 0x3C, 0x00 },
    [0x4A] = { 0x06, 0x06, 0x06, 0x06, 0x06, 0x66, 0x3C, 0x00 },
    [0x4B] = { 0x66, 0x6C, 0x78, 0x70, 0x78, 0x6C, 0x66, 0x00 },
    [0x4C] = { 0x60, 0x60, 0x60, 0x60, 0x60, 0x60, 0x7E, 0x00 },
    [0x4D] = { 0x63, 0x77, 0x7F, 0x6B, 0x63, 0x63, 0x63, 0x00 },
    [0x4E] = { 0x66, 0x76, 0x7E, 0x7E, 0x6E, 0x66, 0x66, 0x00 },
    [0x4F] = { 0x3C, 0x66, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x00 },
    [0x50] = { 0x7C, 0x66, 0x66, 0x7C, 0x60, 0x60, 0x60, 0x00 },
    [0x51] = { 0x3C, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x0E, 0x00 },
    [0x52] = { 0x7C, 0x66, 0x66, 0x7C, 0x6C, 0x66, 0x66, 0x00 },
    [0x53] = { 0x3C, 0x66, 0x30, 0x1C, 0x06, 0x66, 0x3C, 0x00 },
    [0x54] = { 0x7E, 0x18, 0x18, 0x18, 0x18, 0x18, 0x18, 0x00 },
    [0x55] = { 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x00 },
    [0x56] = { 0x66, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x18, 0x00 },
    [0x57] = { 0x63, 0x63, 0x63, 0x6B, 0x7F, 0x77, 0x63, 0x00 },
    [0x58] = { 0x66, 0x66, 0x3C, 0x18, 0x3C, 0x66, 0x66, 0x00 },
    [0x59] = { 0x66, 0x66, 0x3C, 0x18, 0x18, 0x18, 0x18, 0x00 },
    [0x5A] = { 0x7E, 0x06, 0x0C, 0x18, 0x30, 0x60, 0x7E, 0x00 },
    [0x61] = { 0x00, 0x00, 0x3C, 0x06, 0x3E, 0x66, 0x3E, 0x00 },
    [0x62] = { 0x60, 0x60, 0x7C, 0x66, 0x66, 0x66, 0x7C, 0x00 },
    [0x63] = { 0x00, 0x00, 0x3C, 0x60, 0x60, 0x66, 0x3C, 0x00 },
    [0x64] = { 0x06, 0x06, 0x3E, 0x66, 0x66, 0x66, 0x3E, 0x00 },
    [0x65] = { 0x00, 0x00, 0x3C, 0x66, 0x7E, 0x60, 0x3C, 0x00 },
    [0x66] = { 0x1C, 0x30, 0x78, 0x30, 0x30, 0x30, 0x30, 0x00 },
    [0x67] = { 0x00, 0x00, 0x3E, 0x66, 0x66, 0x3E, 0x06, 0x3C },
    [0x68] = { 0x60, 0x60, 0x7C, 0x66, 0x66, 0x66, 0x66, 0x00 },
    [0x69] = { 0x18, 0x00, 0x38, 0x18, 0x18, 0x18, 0x3C, 0x00 },
    [0x6A] = { 0x0C, 0x00, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x38 },
    [0x6B] = { 0x60, 0x60, 0x66, 0x6C, 0x78, 0x6C, 0x66, 0x00 },
    [0x6C] = { 0x38, 0x18, 0x18, 0x18, 0x18, 0x18, 0x3C, 0x00 },
    [0x6D] = { 0x00, 0x00, 0x66, 0x7F, 0x7F, 0x6B, 0x63, 0x00 },
    [0x6E] = { 0x00, 0x00, 0x7C, 0x66, 0x66, 0x66, 0x66, 0x00 },
    [0x6F] = { 0x00, 0x00, 0x3C, 0x66, 0x66, 0x66, 0x3C, 0x00 },
    [0x70] = { 0x00, 0x00, 0x7C, 0x66, 0x66, 0x7C, 0x60, 0x60 },
    [0x71] = { 0x00, 0x00, 0x3E, 0x66, 0x66, 0x3E, 0x06, 0x06 },
    [0x72] = { 0x00, 0x00, 0x7C, 0x66, 0x60, 0x60, 0x60, 0x00 },
    [0x73] = { 0x00, 0x00, 0x3E, 0x60, 0x3C, 0x06, 0x7C, 0x00 },
    [0x74] = { 0x30, 0x30, 0x7C, 0x30, 0x30, 0x30, 0x1C, 0x00 },
    [0x75] = { 0x00, 0x00, 0x66, 0x66, 0x66, 0x66, 0x3E, 0x00 },
    [0x76] = { 0x00, 0x00, 0x66, 0x66, 0x66, 0x3C, 0x18, 0x00 },
    [0x77] = { 0x00, 0x00, 0x63, 0x6B, 0x7F, 0x7F, 0x36, 0x00 },
    [0x78] = { 0x00, 0x00, 0x66, 0x3C, 0x18, 0x3C, 0x66, 0x00 },
    [0x79] = { 0x00, 0x00, 0x66, 0x66, 0x66, 0x3E, 0x06, 0x3C },
    [0x7A] = { 0x00, 0x00, 0x7E, 0x0C, 0x18, 0x30, 0x7E, 0x00 },
};

void gui_draw_text(rsl_fb_t* fb, int x, int y, const char* text, uint32_t color) {
    if (!fb || !fb->address || !text) return;
    int cur_x = x;
    for (int k = 0; text[k]; k++) {
        uint8_t c = (uint8_t)text[k];
        if (c < 128) {
            const uint8_t* glyph = font8x8_basic[c];
            for (int i = 0; i < 8; i++) {
                for (int j = 0; j < 8; j++) {
                    if (glyph[i] & (1 << (7 - j))) {
                        int px = cur_x + j;
                        int py = y + i;
                        if (px >= 0 && (uint64_t)px < fb->width && py >= 0 && (uint64_t)py < fb->height) {
                            uint32_t* pixel = (uint32_t*)(fb->address + py * fb->pitch + px * 4);
                            *pixel = color;
                        }
                    }
                }
            }
        }
        cur_x += 8;
    }
}

char* strstr(const char* haystack, const char* needle) {
    if (!*needle) return (char*)haystack;
    for (; *haystack; haystack++) {
        if (*haystack == *needle) {
            const char *h, *n;
            for (h = haystack, n = needle; *h && *n && *h == *n; h++, n++);
            if (!*n) return (char*)haystack;
        }
    }
    return NULL;
}
